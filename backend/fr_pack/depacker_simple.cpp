...
sBool DecodeBit(DepackState *st,sInt index,sInt move)
{
  sU32 bound;
  sBool result;
...
sInt DecodeTree(DepackState *st,sInt model,sInt maxb,sInt move)
{
  sInt ctx;

  ctx = 1;
  while(ctx < maxb)
    ctx = (ctx * 2) + DecodeBit(st,model + ctx,move);

  return ctx - maxb;
}

sInt DecodeGamma(DepackState *st,sInt model)
{
  sInt value = 1;
  sU8 ctx = 1;
  ...
  do
  {
    ctx = ctx * 2 + DecodeBit(st,model + ctx,5);
    value = (value * 2) + DecodeBit(st,model + ctx,5);
    ctx = ctx * 2 + (value & 1);
  }
  while(ctx & 2);
...
sU32 KKrunchyDepacker(sU8 *dst,const sU8 *src)
{
  ...
  while(1)
  {
    ...
    {
    case 0: // literal
      *dst++ = DecodeTree(&st,LiteralModel,256,4);
      LWM = 0;
      break;
    case 1: // match
      len = 0;
      if(!LWM && DecodeBit(&st,PrevMatchModel,5)) // prev match
        offs = R0;
      else
      {
        offs = DecodeGamma(&st,Gamma0Model);
        if(!offs)
          return dst - dsto;
        offs -= 2;
        offs = (offs << 4) + DecodeTree(&st,MatchLowModel + (offs ? 16 : 0),16,5) + 1;
        if(offs>=2048)  len++;
        if(offs>=96)    len++;
      }
      R0 = offs;
      LWM = 1;      
      len += DecodeGamma(&st,Gamma1Model);
	  while (len--)
	  {
		  *dst++ = dst[-offs];
	  }
    ...
    code = DecodeBit(&st,CodeModel + LWM,5);
  }
  return 0;
}
