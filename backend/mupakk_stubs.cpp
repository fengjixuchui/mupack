...
extern "C"
{
	...
	*/
	...
	static void restore(stubcode* p, INT_PTR base_offset)
	{
		...
		DWORD* Thunk;
		DWORD* Function;
		IMAGE_IMPORT_DESCRIPTOR* Imports = (IMAGE_IMPORT_DESCRIPTOR*)(p->ImageBase + p->OriginalImports);
		while (Imports->Name)
		{
...
#define Test86MSByte(b) ((b) == 0 || (b) == 0xFF)
	...
	static void depack_fnc(stubcode *p, INT_PTR base_offset)
	{
		...
		unsigned char* input = (unsigned char*)valloc(NULL, p->sizepacked, MEM_COMMIT, PAGE_READWRITE);
		memcp1(input, (LPVOID)p->packed_ptr, p->sizepacked);
		vprotect((LPVOID)p->packed_ptr, p->sizeunpacked, PAGE_EXECUTE_READWRITE, &OldP);
		typedef int(_stdcall* tdecomp) (PVOID, PVOID);
		...
		vprotect((LPVOID)p->ImageBase,pNTHeader->FileHeader.SizeOfOptionalHeader , PAGE_READWRITE, &old_protect);
		DWORD offset_to_directories = p->ImageBase + pDosHeader->e_lfanew
...
unsigned char* build_stub(int OEP, int imagebase, int section_va, int * stubsize)
{
	...
	DWORD main_depack = imagebase + section_va + sizeof(stubcode) + sfunc[SIZE_BOOTSTRAP];
	DWORD pointer_tbl = imagebase + section_va + sfunc[SIZE_BOOTSTRAP];
	Bootstrapper shellcode(pointer_tbl, main_depack,imagebase+OEP);
	*stubsize = sizeof(stubcode) + sfunc[SIZE_BOOTSTRAP] + sfunc[SIZE_LOADER] + sfunc[SIZE_RESTORE]
	+ sfunc[SIZE_CODEFILTER] + sfunc[SIZE_DECOMPRESS];
